# TestFlight Notes for Chowser 3.9.4 (Build 202607110318)

## Beta notes

This build covers everything shipped since 3.9.0: Advanced Routing (fallback, multi-source rules, URL rewrites), network privacy defaults, recoverable App Mode transitions, a much larger MCP API surface for AI-driven configuration, a predefined rewrite-rule catalog, a Handoff fix, and privacy-safe lifecycle diagnostics.
...
- **App Mode recovery**: repeatedly switch App → Menu Bar → App, open Settings from both modes, and immediately open a link after each switch. Chowser must preserve the previous reachable mode if macOS rejects a transition.
...
- **Diagnostics** (Settings → General → About → Diagnostics): switch App Mode a few times, confirm the lifecycle timeline records mode/status-item changes, and export a report. The exported report must contain only app/system metadata and typed lifecycle events, never visited hosts, local paths, source apps, clipboard contents, or raw operational logs.
...
- Windows and Linux builds. The former Electrobun prototype has been removed and is no longer supported.
...
- Marketing version: `3.9.4`
- Build number: `202607110318`
...
- Confirm the uploaded build number is `202607110318` and the version is `3.9.4`.
