...
Native-app entries are data, not app-specific Swift code. Source rules use exact lowercase HTTP(S) hosts, fixed path lengths, bounded typed captures, and structured query requirements. Targets use a declared custom scheme plus structured host, path, and query components. Regex, scripts, free-form URL interpolation, credentials, ports, fragments, and reserved schemes such as `file`, `data`, `javascript`, `http`, `https`, and `chowser` are not allowed.
